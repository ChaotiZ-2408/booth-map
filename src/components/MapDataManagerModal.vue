...
import { normalizeMapData, parseMapBooths, validateMapData } from "@/data/mapData";
...
async function loadExampleData() {
  try {
    const [layoutRes, boothsRes] = await Promise.all([
      fetch("/example-data/map-layout.json", { cache: "no-store" }),
      fetch("/data/events/Nice2.json", { cache: "no-store" }),
    ]);
    if (!layoutRes.ok || !boothsRes.ok) throw new Error("load failed");
    const layoutPayload = await layoutRes.json();
    const boothPayload = await boothsRes.json();
    const exampleMapData = {
      version: 1,
      map: {
        layout: layoutPayload.layout ?? layoutPayload,
        booths: parseMapBooths(boothPayload),
      },
    };
    jsonText.value = JSON.stringify(exampleMapData, null, 2);
  } catch {
    alert("載入範例資料失敗");
  }
}
...
function importJson() {
  try {
    const parsed = JSON.parse(jsonText.value) as { map?: unknown };
    ...
    const normalized = normalizeMapData(parsed.map);
    const validationError = validateMapData(normalized);
...
<template>
  <Teleport to="body">
    <div v-if="visible" class="modal-backdrop" @click="visible = false">
      <section class="modal-panel" role="dialog" aria-modal="true" aria-labelledby="map-data-modal-title" @click.stop>
        <header class="modal-header">
          <div class="modal-title-group">
            <h2 id="map-data-modal-title" class="modal-title">地圖資料</h2>
        ...
        <div class="modal-body">
          <p class="modal-description">編輯完整地圖資料，保存前會先檢查地圖結構與攤位資料。</p>

          <textarea v-model="jsonText" class="modal-textarea" aria-label="地圖 JSON 資料" spellcheck="false" />
        </div>

        <footer class="modal-footer">
          <button type="button" class="modal-button modal-button-subtle mr-auto" @click="loadExampleData">載入範例</button>
