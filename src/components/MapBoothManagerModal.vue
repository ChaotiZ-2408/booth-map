...
function openJson() {
  jsonText.value = JSON.stringify(serializeMapBooths(props.booths), null, 2);
}
...
async function loadExampleData() {
  try {
    const response = await fetch("./example-data/map-booths.json", { cache: "no-store" });
...
function importJson() {
  try {
    ...
    try {
      ...
      return alert(e instanceof Error ? e.message : "?澆??航炊");
    }
    ...
    <div v-if="visible" class="fixed inset-0 z-50 flex items-end bg-black/30 backdrop-blur-sm" @click="visible = false">
      <div class="flex h-[85vh] w-full flex-col rounded-t-[28px] bg-white/80 p-3 backdrop-blur-xl" @click.stop>
        <div class="mb-3 flex items-center justify-between">
          <div>
            <div class="text-base font-semibold text-slate-700">攤位資料</div>
        ...
        <div class="mt-3 grid grid-cols-2 gap-2">
          <button type="button" class="glass-button" @click="copyJson">複製 JSON</button>
